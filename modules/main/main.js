...
async function getWeather(
  ...
  if (!isAutoUpdate) {
    ...
    result.innerHTML = `
          <div class="loader-container">
              ...
              <div id="slow-conn-msg" class="slow-connection-text">Slow Connection</div>
          </div>
      `;
  ...
  try {
    ...
    if (!hasGeoData) {
      ...
      try {
        ...
        if (isZip) {
          ...
          */
          const owmZipRes = await fetch(`/api/weather?action=geo_zip&zip=${encodeURIComponent(cityInput)}`);
          if (owmZipRes.ok) {
        ...
        if (!hasGeoData) {
          ...
          */
          const owmRes = await fetch(`/api/weather?action=geo_direct&q=${encodeURIComponent(cityInput)}&limit=1`);
    ...
    */
    ...
    try {
      const alertsRes = await fetch(`/api/weather?action=alerts&lat=${lat}&lon=${lon}`);
      if (alertsRes.ok) {
    ...
    result.innerHTML = `
          <div class="weather-main-display">
              <div class="weather-info">
                  ...
                  <div class="feels-like">Feels like ${Math.round(displayFeelsLike)}${tempUnit.toLowerCase()}<span style="opacity: 0.7; margin: 0 8px;">&bull;</span><span style="font-family: Arial, sans-serif; opacity: 0.9">${todayMaxTempDisplay}&deg; / ${todayMinTempDisplay}&deg;</span></div>
              </div>
